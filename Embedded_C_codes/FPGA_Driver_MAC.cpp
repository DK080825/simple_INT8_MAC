...
#define MAC_IP_BASE_PHYS   0x00A0000000LL


#define REG_MMAP_SIZE      0x00001000LL   

typedef uint64_t U64;
typedef uint32_t U32;

volatile struct {
    U64  reg_phys;        // Physical base address for register (informational)
...
int mac_ip_open() {
    ...
    for (int dir = 0; dir < num_dirs; ++dir) {
        ...
        if (is_target_dev(namelist[dir]->d_name, UIO_TARGET)) {
            ...
            printf("Opened device: %s (%s)", path, UIO_TARGET);

            MAC_IP_info.reg_phys = MAC_IP_BASE_PHYS;
            MAC_IP_info.pio_32_mmap = (U32*)mmap(NULL, REG_MMAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd_reg, 0);
            if (MAC_IP_info.pio_32_mmap == MAP_FAILED) {
                perror("mmap failed");
                close(fd_reg);
                free(namelist);
                return -1;
