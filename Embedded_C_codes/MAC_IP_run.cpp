...
static inline void write_wgt(uint16_t addr, int8_t data) {
    ...
    reg_write32(MAC_WGT_DATA_OFF, (uint8_t)data); // keep your original behavior
...
/*
 * Compute M0_int given that n is already chosen externally.
 *
 * Real multiplier:
 *   M = (Sx * Sw) / Sy
 *
...
int main() {
    ...
    // ------------------------------------------------------------
    // 1) Testcase constants (ONE CHANNEL IN / ONE CHANNEL OUT)
    //    You asked: compute Zo and n outside => DO NOT derive them here.
    // ------------------------------------------------------------
    const uint32_t NUM  = 9;

    // From your Python run (example values; replace only if your Python prints different)
    const uint8_t  Za   = 60;   // Zx (input zero_point)
    const uint8_t  Zw   = 0;    // Zw (weight zero_point)
    const uint8_t  Zo   = 55;   // Zy (output zero_point)  <-- fixed externally
    const uint32_t n    = 8;    // shift amount            <-- fixed externally
    ...
    // ------------------------------------------------------------
    // 3) INPUT x_q (uint8) 8x8  (ONE CHANNEL)
    // ------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // 4) WEIGHTS w_q (int8) 3x3 (ONE KERNEL)
    // ------------------------------------------------------------
    const int8_t w[3][3] = {
        {  73, -17,   5},
        {  31,  82, 127},
    ...
    // Load weights once into WGT[0..8]
    int idx = 0;
    for (int r = 0; r < 3; r++) {
        for (int c = 0; c < 3; c++) {
    ...
    // ------------------------------------------------------------
    // 5) EXPECTED y_q (uint8) 8x8 (ONE CHANNEL)
    // ------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // 6) Run conv: 8x8 output, 3x3 kernel, pad=1, stride=1
    ...
    // ------------------------------------------------------------
    // 7) Print EXPECTED vs ACTUAL + mismatch count
    // ------------------------------------------------------------
